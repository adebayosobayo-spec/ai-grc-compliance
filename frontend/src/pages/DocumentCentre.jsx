...
const DOCUMENT_TYPES = [
    {
        ...
        icon: '📑',
        description: 'Documents which ISO controls apply to your organisation and which are excluded, with justification. Required for ISO 27001 certification.',
        columns: ['Control ID', 'Control Name', 'Applicable', 'Justification', 'Implementation Status'],
        required: true,
    },
    {
        id: 'incident',
    ...
    },
    {
        ...
        icon: '🤝',
        description: 'Tracks third-party vendors/suppliers who access your data or systems, their risk level, and contract/SLA details. ISO 27001 A.5.19–A.5.23.',
    ...
    },
    {
        ...
        icon: '🎓',
        description: 'Records security awareness training for all staff. ISO 27001 A.6.3 requires competence and awareness programmes with evidence of completion.',
...
function DocumentCard({ doc, entries, onAddEntry, onExport }) {
                ...
                <div className="border-t border-slate-100 p-4">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {entries.map((entry, i) => (
                                        <tr key={i} className="hover:bg-slate-50">
                                            {doc.columns.map(col => (
                                                <td key={col} className="px-3 py-2 text-xs text-slate-700">{entry[col] || '—'}</td>
                                            ))}
                                        </tr>
                                    ))}
                                </tbody>
                    ...
                    {showForm ? (
                        <div className="bg-slate-50 rounded-lg p-4 space-y-3">
                            <p className="text-xs font-semibold text-slate-700 mb-2">Add New Entry</p>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                {doc.columns.map(col => (
                                    <div key={col}>
                                        ...
                                        <input
                                            value={formRow[col] || ''}
                                            onChange={e => setFormRow(f => ({ ...f, [col]: e.target.value }))}
                            ...
                            <div className="flex gap-2 mt-3">
                                <button onClick={handleAdd}
                                    className="px-3 py-1.5 text-xs font-medium text-white bg-primary-600 rounded hover:bg-primary-700">Add</button>
...
export default function DocumentCentre() {
    ...
    const framework = orgProfile?.compliance_framework === 'ISO_42001' ? 'ISO 42001' : 'ISO 27001'
    const orgName = orgProfile?.organization_name || 'Your Organisation'

    function addEntry(docId, row) {
        setAllEntries(prev => ({
            ...prev,
            [docId]: [...(prev[docId] || []), row],
        }))
    }
        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                    {framework} compliance requires maintaining several operational registers and records beyond policies.
                    This is your central hub for all compliance documentation.
                </p>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p className="text-xs text-blue-700 mt-2">
                    ISO certification auditors will ask for evidence of <strong>operational processes</strong>, not just policies.
                    These registers prove that your organisation is actively managing risks, incidents, suppliers, and staff training.
                    Documents marked <span className="bg-red-50 text-red-600 px-1 rounded text-[10px] font-bold">REQUIRED</span> are
                    mandatory for certification. Start with the most critical ones and build over time.
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: 'Total Documents', value: `${docsWithEntries} / ${totalDocs}` },
                    { label: 'Required Completed', value: `${requiredCompleted} / ${requiredDocs}` },
